...
function Query( opts ) {
	...
	// [9] Initialize a request cache which is used to track multi-part and pending requests:
	this._cache = {};

	// [10] Initialize a request count (used for assigning query ids):
	this._qid = 0;

	// [11] Add event listeners to keep track of active queries...
	this.on( 'init', init );
	this.on( 'end', end );

	// [12] Start querying the endpoint on the next tick...
	if ( poll ) {
		setTimeout( start, 0 );
	} else {
